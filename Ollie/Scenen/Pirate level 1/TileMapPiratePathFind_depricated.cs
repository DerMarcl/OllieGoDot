...
public partial class TileMapPiratePathFind : TileMap
{
	[Export]
	...
	private Node _debugDrawer;
	// Called when the node enters the scene tree for the first time.
	public override void _Ready()
	{
		var debugDrawer = GetNode("../DebugDrawer");  // relative to TileMap
		_debugDrawer = debugDrawer;
		// Load in the graph point packed scene
		_graphpoint = ResourceLoader.Load<PackedScene>("res://Scenen/GraphPoint.tscn");
	...
	public Godot.Collections.Array<Godot.Collections.Dictionary> GetPlatform2DPath(Vector2 startPos, Vector2 endPos)
{
	var rawStack = InternalPlatform2DPath(startPos, endPos); // your existing stack-based method
	var result = new Godot.Collections.Array<Godot.Collections.Dictionary>();

	foreach (var point in rawStack)
	{
		var dict = new Godot.Collections.Dictionary
		{
			{ "position", point.Position },
			{ "is_left_edge", point.IsLeftEdge },
			{ "is_right_edge", point.IsRightEdge },
			{ "is_fall_tile", point.IsFallTile },
			{ "is_right_wall", point.IsRightWall },
			{ "is_left_wall", point.IsLeftWall }
		};
		result.Add(dict);
	}
	...
	public System.Collections.Generic.Stack<PointInfo> InternalPlatform2DPath(Vector2 startPos, Vector2 endPos)
	{
	...
	{
		if (!ShowDebugGraph || _debugDrawer == null)
		return;
		_debugDrawer.Call("add_line", from, to, color);
	...
	{
		var localPos = GetGlobalTileCenter(tile);                            // Map the position to screen coordiantes
	...
	private void AddVisualPoint(Vector2I tile, Color? color = null, float scale = 1.0f)
	{
		...
		visualPoint.Position = GetGlobalTileCenter(tile);    // Map the position of the visual point to local coordinates
	...
	private PointInfo GetPointInfo(Vector2I tile)
	{
		...
		foreach (var pointInfo in _pointInfoList)
		{
			...
			if (pointInfo.Position == GetGlobalTileCenter(tile))
			{
	...
	private void AddFallPoint(Vector2I tile)
	{
		...
		var fallTileLocal = (Vector2I)GetGlobalTileCenter((Vector2I)fallTile);                       // Get the local coordinates for the fall tile
	...
	private void AddLeftEdgePoint(Vector2I tile)
	{
		...
		if (GetCellSourceId(COLLISION_LAYER, new Vector2I(tile.X - 1, tile.Y)) == CELL_IS_EMPTY)
		{
			...
			{
				long pointId = _astarGraph.GetAvailablePointId();                       // Get the next available point id
				var pointInfo = new PointInfo(pointId, (Vector2I)GetGlobalTileCenter(tileAbove));// Create a new point information, and pass in the pointId
				pointInfo.IsLeftEdge = true;                                            // Flag that the tile is a left edge
				_pointInfoList.Add(pointInfo);                                          // Add the tile to the point info list
				_astarGraph.AddPoint(pointId, (Vector2I)GetGlobalTileCenter(tileAbove));         // Add the point to the Astar graph, in local coordinates
	...
	private void AddLeftWallPoint(Vector2I tile)
	{
		...
		if (GetCellSourceId(COLLISION_LAYER, new Vector2I(tile.X - 1, tile.Y - 1)) != CELL_IS_EMPTY)
		{
			...
			if (existingPointId == -1)
			{
				...
				_pointInfoList.Add(pointInfo);                                          // Add the tile to the point info list
				_astarGraph.AddPoint(pointId, (Vector2I)GetGlobalTileCenter(tileAbove));         // Add the point to the Astar graph, in local coordinates
				AddVisualPoint(tileAbove, new Color(1, 1, 0, 1));                       // Add a black point to the map (if ShowDebugGraph = true)
			}
			else
			{
	...
	private Vector2 GetGlobalTileCenter(Vector2I tile)
	{
