...
func _physics_process(delta):
	...
	direction = Input.get_axis("Left_second_player", "Right_second_player")
	...
	if power_state != GameManager.PossiblePowers.NORMAL and Input.is_action_just_pressed("Action_second_player"):
	...
	if Input.is_action_just_pressed("Jump_second_player") and not does_spcial_action and ((is_on_floor() or coyote_timer > 0.0) or (is_on_wall_only() and direction and (has_walljump or has_backwards_walljump))):
		velocity.y = JUMP_VELOCITY 
		sfx_jump.play()
	...
	if Input.is_action_just_pressed("HideKevin"):
		HideKevin()
